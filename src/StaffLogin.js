...
export default function StaffLogin(props) {
  ...
  //  const [employee, setemployee] = useState({ Email: '', Password: ''});  
    ...
    const onSubmit = (data) => 
    {
         ...
         axios.post("https://localhost:44390/api/StaffRegister/Login?Email="+data.Email+"&Password="+data.Password)
           .then(function (response) {
               //alert('Successfully submitted');
             localStorage.setItem('staff:token',JSON.stringify(response.data))
            //console.log("A:",localStorage.setItem('user:token',response));
             console.log(localStorage.getItem('staff:token'))
             alert('Successfully submitted');
    ...
    return (
        ...
        <div className="container border border-dark">
         ...
         <h1>Sign-in</h1>
            ...
            <div>
                <h4>Create an account?<Link to="/staffregister">Register</Link></h4>
            </div>
        </form>
        </div>
        </div>
